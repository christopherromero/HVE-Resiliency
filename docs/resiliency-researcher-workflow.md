...
## Optional Manual Assessment Utilities

The planning pipeline does not run post-processing prompts. Invoke `/detect-assessment-duplicates` manually to identify findings that restate the same verified root issue. After resolving consolidation decisions, invoke `/reorder-assessment-findings` manually to place GLB health-probe work first and renumber each priority tier in validated waves.
