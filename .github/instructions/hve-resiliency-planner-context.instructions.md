...
* It reflects the current implementation of this repository with respect to regional failover between West US 2 and West US
...
If an issue **only manifests because of a failure event** (region failure, GLB failover), it qualifies as a resiliency finding, even if a procedural or manual workaround exists. The existence of a workaround affects **priority** (P1 vs P0), not **categorization**.
