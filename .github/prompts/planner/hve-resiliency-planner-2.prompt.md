...
- Source fidelity: the "before" (current-state) code in every example must be a verbatim quote of the source at the cited file:line (re-open the file to confirm), following the Source Fidelity and Evidence Verification rule in the Task Planner Context.
...
4) Findings and Recommended Patterns (grouped and ordered P0 then P1 then P2 then P3). Use the following template per finding:
   ...
   - Code examples (repo's language) and where to apply them: provide explicit code for every affected location/scenario in this finding, each with its file and line. The current-state ("before") code must be quoted verbatim from source per the Source Fidelity rule. Do not use a single example with a note to replicate it elsewhere.
