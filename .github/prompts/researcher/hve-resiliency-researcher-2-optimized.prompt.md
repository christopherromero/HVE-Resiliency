...
# Application HVE Researcher 2 Optimized
...
2. Require Prompt 1a `A) In-Scope Azure Services (Definitive List)` and Prompt
   1b `Section 1 - Used External Dependencies (Evidence Confirmed)`. Treat only
   these sections as the confirmed dependency inventories for Prompt 2.
3. Exclude Prompt 1a section B and Prompt 1b Sections 2 and 3. Do not analyze a
   dependency absent from both confirmed inventories.
4. Stop as `Blocked` before repository traversal if an input is missing,
   unreadable, belongs to another repository, lacks the required inventory, or
   makes the allowed dependency scope ambiguous. Report the exact prerequisite
   problem and request the corrected path. Do not write the research artifact
   or suggest Prompt 3.
