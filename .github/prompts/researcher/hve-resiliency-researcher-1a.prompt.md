...
Any incomplete manifest, read or scan failure, clipping, sanitization uncertainty, lost or unreconciled ID, package failure, count or hash mismatch, or failed audit makes the run incomplete. Never convert such a failure into a negative or Not Applicable result. Use one of these failure codes: `MANIFEST_INCOMPLETE`, `SCAN_FAILED`, `SANITIZATION_FAILED`, `RECONCILIATION_FAILED`, or `COVERAGE_FAILED`. For incomplete execution, render only the exact incomplete form below.
