...
- Per the Database-to-Kafka Pairing Standard, an Active-Active Kafka topology is expected to pair with an Active-Active (multi-master) database; flag any use of a single-master/Active-Standby database (for example Azure SQL) alongside this Active-Active Kafka design as a pairing mismatch

Focus specifically on:
- Kafka bootstrap and DNS usage, including `advertised.listeners`-driven broker metadata caching: verify the application does not persist or hard-code broker-specific host:port addresses learned from cluster metadata that would bypass bootstrap DNS resolution after a broker restart or regional cutover
...
- Database-to-Kafka pairing: does the repository's confirmed database resiliency model (multi-master vs single-master) match the Active-Active Kafka topology assumed by this prompt?
