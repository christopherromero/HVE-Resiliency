...
* Has the expected producer shape and all required fields. Prompt 1a must contain `Section 1 - Used External Dependencies (Evidence Confirmed)`, `Section 2 - Checked but Not Present`, and `Section 3 - Not Applicable`. Its Section 1 is the evidence-confirmed Azure dependency set and each entry must include service name, Azure service category, evidence class, file-line evidence, factual use, and region or failover sensitivity. Prompt 1b must contain `Section 1 — Used External Dependencies (Evidence Confirmed)`, `Section 2 — Checked but Not Present`, and `Section 3 — Not Applicable`; each Section 1 entry must include service or dependency name, file-line evidence, factual use, failover impact, mitigations, health-check and GLB-signaling details, and constraints or limitations.
* Is structurally complete: all producer-required sections are present, required entries are well-formed, and no blocked, incomplete, failed, unresolved, clipping, read-error, or sanitization-failure state remains. Do not require a producer metadata field that Prompt 1a or Prompt 1b does not emit.
...
If either artifact is missing, outside the allowed directory, incomplete, stale, ambiguous, malformed, or incompatible, select `Blocked: prerequisite incomplete`. Stop without production discovery, broad fallback, or an empty findings result.
