...
Run Prompt 7 only; do not launch earlier prompts. An artifact in the inherited research location qualifies only when it starts with YAML frontmatter containing these values and has heading `## SECTION 1 - USED DEPENDENCIES (EVIDENCE CONFIRMED)`:

* Prompt 1a: `source-prompt: hve-resiliency-researcher-1a`, `schema-version: 1`, `status: current`, and `## Section 1 - Used External Dependencies (Evidence Confirmed)`
